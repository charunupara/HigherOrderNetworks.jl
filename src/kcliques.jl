...
The Chiba-Nishizeki algorithm for finding all traingles (3-cliques) in a graph. Found on pp. 212-213 of the paper

Arguments
---------
    - `A::MatrixNetwork`: The graph in which to search for triangles

To improve the runtime of the brute-force way of searching for triangles, this algorithm first sorts the adjacency list by degree, then, 
after finding all triangles the current vertex is a part of, it removes the vertex in order to quickly arrive at a relatively sparse graph.
The runtime of this algorithm is represented in terms of the graph's arboricity, which is the minimum number of forests the edges can be partitioned into.
"""
function triangles(A::MatrixNetwork)
    A = AdjacencyListGraph(A)
    new_graph = deepcopy(A)
    sorted = sort(1:A.n, by=x -> new_graph.D[x], rev=true)
    cliques = Vector{Vector{Int64}}()
    tri_edges = deepcopy(A.list)
    marked_nodes = falses(A.n)

    for v in sorted[1:A.n - 2]
        cur = new_graph.list[v]
        ...
        for u in cur
            for w in new_graph.list[u]
                if marked_nodes[w]
                    cur_clique = [v, u, w]
                    push!(cliques, cur_clique)
                    filter!(x -> x != u, tri_edges[v])
                    filter!(x -> x != w, tri_edges[u])
                    filter!(x -> x != v, tri_edges[w])

                    filter!(x -> x != v, tri_edges[u])
                    filter!(x -> x != u, tri_edges[w])
                    filter!(x -> x != w, tri_edges[v])
                end
            marked_nodes[u] = false
            end
        end

        # delete node
        for n_list in new_graph.list
            filter!(x -> x ≠ v, n_list)
        end
        new_graph.list[v] = []
        deleteat!(sorted, 1)
    end
    return cliques, map(collect, collect(Set([Set([i,v]) for i = 1:size(tri_edges,1) for v in tri_edges[i]])))
end
